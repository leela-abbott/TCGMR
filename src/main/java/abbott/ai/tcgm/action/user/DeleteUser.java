 ...
 * @version 1.0
 */
public class DeleteUser extends TCGMAction
{
	/**
	 * Default Constructor
	 */
	...
	public ActionForward perform(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response)throws IOException, ServletException
	{
		...
		else if(this.isSessionValid(request))
		{
			...
			try
			{
				...
				int adminCode = userMngr.deleteUsers(userToken,userForm.getUserlist());
				if (adminCode ==1)
				{
			...
			catch(TCGMException tcgme)
			{
				...
				this.errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("exception.user.delete"));
