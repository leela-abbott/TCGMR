 ...
 * @version 1.0
 */
public class SaveUser extends TCGMAction
{
	/**
	 * Default Constructor
	 */
	...
	public ActionForward perform(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response)throws IOException, ServletException
	{
		...
		if(form == null)
		{
			...
			errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("error.user.form.missing"));
		...
		else if(this.isSessionValid(request))
		{
			...
			UserForm userForm = (UserForm)form;//cast the form that was passed in to the correct type for this action
			...
			try
			{
				if ((!(userForm.getUserToEdit().getUserid() == null)) && 
				       (userForm.getUserToEdit().getUserid().equalsIgnoreCase(userToken.getUserid())))
				{
					this.errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("exception.admin.delete"));
				}
				else
				{
					userMngr.saveUser(userToken,userForm.getUserToEdit());
				}
