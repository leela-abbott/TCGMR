 ...
 * @version 1.0
 */
public class CloseModel extends TCGMAction
{
	/**
	 * @param mapping
	 * @param form
	...
	public ActionForward perform(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response)throws IOException, ServletException
	{
		try
		{
			if ( this.isSessionValid(request) )
			{
				...
				{
					errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("error.model.close.job_pending") );
				...
				else
				{
					...
					model.setStatus(TCGMModel.Status.CLOSED);
