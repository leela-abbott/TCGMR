package abbott.ai.tcgm.action.knollconv;
 ...
 * @version 1.0
 */
public class DeleteKnollConv extends TCGMAction
{
	/**
	 * Default Constructor
	 */
	public DeleteKnollConv()
	{
		super();
	}
	...
	public ActionForward perform(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response)throws IOException, ServletException
	{
		...
		if(form == null)
		{
			...
			errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("error.knollconv.form.missing"));
		...
		else if(this.isSessionValid(request))
		{
			...
			KnollConvForm knollConvForm = (KnollConvForm)form;//cast the form that was passed in to the correct type for this action

			knollConvForm.processCmd(mapping,request);

			KnollConvMngr knollConvMngr = new KnollConvMngr(); //create the helper class that will handle the processing

			try
			{
				knollConvMngr.deleteKnollConv(userToken,knollConvForm.getKnollConvList());
				knollConvForm.setKnollConvToEdit(new KnollConv());
				knollConvForm.setKnollConvList(knollConvMngr.getKnollConv(userToken,knollConvForm.getSortObject()));
				this.setForward(TCGMConstants.FORWARD_SUCCESS);
			}
			catch(TCGMException tcgme)
			{
				this.logger.error(tcgme.toString(),tcgme);
				//this.errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("exception.knollconv.delete"));
