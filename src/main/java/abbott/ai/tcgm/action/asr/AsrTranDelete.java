 ...
 * @version 1.0
 */
public class AsrTranDelete extends TCGMAction
{
	/**
	 * Default Constructor
	 */
	public AsrTranDelete()
	{
		super();
	}
	/**
		...
		else if(this.isSessionValid(request))
		{
			...
			if(this.isModelSelected(request))
			{
				...
				try
				{
					...
					/*************************************************************
					*	Added by Gain on 03/22/2006 to provide the user(Analyst)
					...
					**************************************************************/
					...
					{
						asrMngr.deleteAllAsrTran(userToken,asrTranForm.getSearchObject());
					}
					else if(asrTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_DELETE_SELECTED) )
					{
						asrMngr.deleteSelectedAsrTran(userToken,asrTranForm.getAsrTranList());
					}
					else if(! asrTranForm.getCmd().trim().equals(""))
					{
						throw new TCGMException(className,methodName,"Invalid command in action");
					}
					errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("success.asrtran.delete"));
