 ...
 * @version 1.0
 */
public class AsrTranCopy extends TCGMAction
{
	/**
	 * Default Constructor
	 */
		...
		else if(this.isSessionValid(request))
		{
			...
			if(this.isModelSelected(request))
			{
				...
				try
				{
					...
					*   the option to Copy(from One Model to Other) the records of any user. Start
					**************************************************************/
					...
					/*************************************************************
					*	Added by Uday on 03/21/2006 to provide the user(Analyst)
					*   the option to Copy(from One Model to Other) the records of any user. End
					**************************************************************/

					if(asrTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_COPY_ALL) )
					{
						asrMngr.copyAllAsrTran(userToken,asrTranForm.getSearchObject(),asrTranForm.getModelSelected());
					}
					else if(asrTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_COPY_SELECTED) )
					{
						asrMngr.copySelectedAsrTran(userToken,asrTranForm.getAsrTranList(),asrTranForm.getModelSelected());
					}
					else if(! asrTranForm.getCmd().trim().equals(""))
					{
						throw new TCGMException(className,methodName,"Invalid command in action");
					}
					errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("success.asrtran.copied"));
