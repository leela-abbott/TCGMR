package abbott.ai.tcgm.action.prmfg;
 ...
 * @version 1.0
 */
public class PRMfgMaint extends TCGMAction
{
	/**
	 * Default Constructor
	 */
	...
	public ActionForward perform(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response)throws IOException, ServletException
	{
		...
		if(form == null)
		{
			...
			errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("error.prmfg.form.missing"));
			this.setForward(TCGMConstants.G_FORWARD_SELECT_MODEL);
		}
		else if(this.isSessionValid(request))
		{
				PRMfgForm prMfgForm = (PRMfgForm)form;//cast the form that was passed in to the correct type for this action
				UserToken userToken = this.getUserToken(request);
				prMfgForm.processCmd(mapping,request);

				PRMfgMngr prMfgMngr = new PRMfgMngr(); //create the helper class that will handle the processing

				try
				{
					prMfgForm.setPrmfglist(prMfgMngr.getPRMfg(userToken,prMfgForm.getSortObject()));
					request.getSession().setAttribute("prMfgForm", prMfgForm);
					request.getSession().setAttribute("prList", prMfgForm.getPrmfglist());
				...
				catch(TCGMException tcgme)
				{
					...
					//this.errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("exception.prmfg.maintenance"));
