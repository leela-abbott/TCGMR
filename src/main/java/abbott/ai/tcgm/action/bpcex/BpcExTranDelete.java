 ...
 * @version 1.0
 */
public class BpcExTranDelete extends TCGMAction {
	/**
	 * Default Constructor
	 */
	public BpcExTranDelete() {
		super();
	}
	/**
	 *
	...
	public ActionForward perform(
		...
		if (form == null) {
			...
			errors.add(
				...
				new ActionError("error.bpcextran.form.missing"));
			...
			if (this.isModelSelected(request)) {
				...
				**************************************************************/
					...
					if (bpXTrnFrm
						.getCmd()
						.equals(TCGMConstants.URL_PARM_VAL_DELETE_ALL)) {

						bpcExMngr.deleteAllBpcExTran(
							userToken,
							bpXTrnFrm.getSearchObject());
					} else if (
						bpXTrnFrm.getCmd().equals(
							TCGMConstants.URL_PARM_VAL_DELETE_SELECTED)) {
						bpcExMngr.deleteSelectedBpcExTran(
							userToken,
							bpXTrnFrm.getBpcExTranList());
					} else if (!bpXTrnFrm.getCmd().trim().equals("")) {
						throw new TCGMException(
							className,
							methodName,
							"Invalid command in action");
					}
					errors.add(
						ActionErrors.GLOBAL_ERROR,
						new ActionError("success.bpcextran.delete"));
				...
				//				}
					...
					errors.add(
						ActionErrors.GLOBAL_ERROR,
						new ActionError("error.update.with.blank.username"));
					try {
						bpXTrnFrm.initModelAndDataset(
							this.getState(request).getCurrentModelIdString(),
							DBConst.DEF_DATASET_TABLE_ID);
						bpXTrnFrm.getPagingFilter().setTotalRecordsInSet(
							bpcExMngr.getCount(
								userToken,
								bpXTrnFrm.getSearchObject()));
						bpXTrnFrm.setBpcExTranList(
							bpcExMngr.getBpcExTran(
								userToken,
								bpXTrnFrm.getSearchObject(),
								bpXTrnFrm.getPagingFilter(),
								bpXTrnFrm.getSortObject()));
						bpXTrnFrm.getSearchObject().getBpcEx().getCreateLog().setUserName(userToken.getUserid());
						this.setForward(TCGMConstants.FORWARD_SUCCESS);
		...
		this.logger.debug("BpcExTranDelete Forward: " + this.getForward());
