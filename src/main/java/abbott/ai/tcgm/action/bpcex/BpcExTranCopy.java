 ...
 * @version 1.0
 */
public class BpcExTranCopy extends TCGMAction
{
	/**
	 * Default Constructor
	 */
	public BpcExTranCopy()
	{
		super();
	}
	/**
		...
		else if(this.isSessionValid(request))
		{
			...
			if(this.isModelSelected(request))
			{
				...
				try
				{
					...
					*   the option to Copy(from One Model to Other)
					*   the records of any user. Start
					**************************************************************/
					...
					*	Added by Uday on 03/21/2006 to provide the user(Analyst)
					*   the option to Copy(from One Model to Other)
					*   the records of any user. End
					**************************************************************/

					if(bpXTrnFrm.getCmd().equals(TCGMConstants.URL_PARM_VAL_COPY_ALL) )
					{
						bpcExMngr.copyAllBpcExTran(userToken,bpXTrnFrm.getSearchObject(),bpXTrnFrm.getModelSelected());
					}
					else if(bpXTrnFrm.getCmd().equals(TCGMConstants.URL_PARM_VAL_COPY_SELECTED) )
					{
						bpcExMngr.copySelectedBpcExTran(userToken,bpXTrnFrm.getBpcExTranList(),bpXTrnFrm.getModelSelected());
					}
					else if(! bpXTrnFrm.getCmd().trim().equals(""))
					{
						throw new TCGMException(className,methodName,"Invalid command in action");
					}
					errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("success.bpcextran.copied"));
