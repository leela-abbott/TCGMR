 ...
 * @version 1.0
 */
public class BpcExTranSave extends TCGMAction
{
	/**
	 * Default Constructor
	 */
	public BpcExTranSave()
	{
		super();
	}
	/**
	...
	public ActionForward perform(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response)throws IOException, ServletException
	{
			...
			if(this.isModelSelected(request))
			{
				...
				try
				{
					...
					**************************************************************/
					...
					if(bpXTrnFrm.getCmd().equals(TCGMConstants.URL_PARM_VAL_ADD) )
					{
						bpcExMngr.addNewBpcExTran(userToken,bpXTrnFrm.getAddNew());
						errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("success.bpcextran.saved"));
					}
					else if(bpXTrnFrm.getCmd().equals(TCGMConstants.URL_PARM_VAL_SAVE_SELECTED) )
					{
						bpcExMngr.saveSelectedBpcExTran(userToken,bpXTrnFrm.getBpcExTranList());
						errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("success.bpcextran.saved"));
					}
					else if(bpXTrnFrm.getCmd().equals(TCGMConstants.URL_PARM_VAL_MASS_UPDATE))
					{
						bpcExMngr.massUpdate(userToken,bpXTrnFrm.getSearchObject(),bpXTrnFrm.getAddNew());
						if(bpXTrnFrm.getBpcExTranListSize() > 0)
						{
							errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("success.bpcextran.saved"));
						}					
					}
		...
		this.logger.debug("BpcExTranSave Forward: " + this.getForward());
