 ...
 * @version 1.0
 */
public class BpcExMaint extends TCGMAction
{
	/**
	 * Default Constructor
	 */
	...
	public ActionForward perform(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response)throws IOException, ServletException
	{
		...
		else if(this.isSessionValid(request))
		{
			if(this.isModelSelected(request))
			{
				...
				if (bpcExForm!=null && bpcExForm.getCmd().equalsIgnoreCase(TCGMConstants.URL_PARM_VAL_ADV_FILTER) )  {
						request.getSession().setAttribute("bpcExTranAdvFilter", bpcExForm);
						bpcExForm.reset(mapping,request);
				...
				try
				{
					...
					if(bpcExForm.getErrs().equalsIgnoreCase("on")){
					...
					if (bpcExForm.getCmd().equalsIgnoreCase(TCGMConstants.URL_PARM_VAL_EMPTY))
					{
					...
					bpcExForm.setBpcExErrorList(new Vector());
					}
					bpcExForm.getAddNew().getBpcEx().setBegPeriod("1");
					bpcExForm.getAddNew().getBpcEx().setEndPeriod("12");
