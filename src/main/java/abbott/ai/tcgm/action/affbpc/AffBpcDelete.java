 ...
 * @version 1.0
 */
public class AffBpcDelete extends TCGMAction
{
	/**
	 * Default Constructor
	 */
...
//			{
				...
				try
				{
					if(affBpcForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_DELETE_ALL) )
					{
//						affBpcMngr.addAllAffBpcToTrans(this.getUserToken(request),affBpcForm.getSearchObject(),TCGMConstants.ACT_CD_DEL);
						affBpcMngr.deleteAllAffBpc(this.getUserToken(request),affBpcForm.getSearchObject());
					}
					else if(affBpcForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_DELETE_SELECTED) )
					{
						affBpcMngr.deleteSelectedAffBpc(this.getUserToken(request),affBpcForm.getAffBpcList());
					}
					else if(! affBpcForm.getCmd().trim().equals(""))
					{
						throw new TCGMException(className,methodName,"Invalid command in action");
					}
					errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("success.affBpc.delete"));

//					affBpcForm.initModelAndDataset(this.getState(request).getCurrentModelIdString(),DBConst.DEF_DATASET_TABLE_ID);
