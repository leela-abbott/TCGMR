 ...
 * @version 1.0
 */
public class BpcsTranPublish extends TCGMAction
{
	/**
	 * Default Constructor
	 */
	public BpcsTranPublish()
	{
		super();
	}
	/**
		...
		else if(this.isSessionValid(request))
		{
			...
			if(this.isModelSelected(request))
			{
				...
				try
				{
					...
					**************************************************************/
					if(bpcsTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_PUBLISH_SELECTED) )
					{
						bpcsMngr.publishSelectedBpcsTran(userToken,bpcsTranForm.getBpcsTranList(), true);
					}
					else if(bpcsTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_UNPUBLISH_SELECTED) )
					{
						bpcsMngr.publishSelectedBpcsTran(userToken,bpcsTranForm.getBpcsTranList(), false);
					}
					else if(bpcsTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_PUBLISH_ALL) )
					{
						bpcsMngr.publishAllBpcsTran(userToken,bpcsTranForm.getSearchObject(), true);
					}
					else if(bpcsTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_UNPUBLISH_ALL) )
					{
						bpcsMngr.publishAllBpcsTran(userToken,bpcsTranForm.getSearchObject(), false);
					}
					else if(! bpcsTranForm.getCmd().trim().equals(""))
					{
						throw new TCGMException(className,methodName,"Invalid command in action");
					}
					if(bpcsTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_PUBLISH_ALL)||
						bpcsTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_PUBLISH_SELECTED)){
						errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("success.bpcsTran.published"));
					   }else{
						errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("success.bpcsTran.unpublished"));
					   }
