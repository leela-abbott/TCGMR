 ...
 * @version 1.0
 */
public class BpcsMaint extends TCGMAction
{
	/**
	 * Default Constructor
	 */
	...
	public ActionForward perform(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response)throws IOException, ServletException
	{
		...
		else if(this.isSessionValid(request))
		{
			if(this.isModelSelected(request))
			{
				...
				if (bpcsForm!=null && bpcsForm.getCmd().equalsIgnoreCase(TCGMConstants.URL_PARM_VAL_ADV_FILTER) )  {
						request.getSession().setAttribute("bpcsTranAdvFilter", bpcsForm);
					    bpcsForm.reset(mapping,request);
				...
				try
				{
					...
					if(bpcsForm.getErrs().equalsIgnoreCase("on")){
					 ...
					 */
					if(bpcsForm.getCmd().equalsIgnoreCase("")){ 
					...
					bpcsForm.setBpcsErrorList(new Vector());
					}
					bpcsForm.getAddNew().getBpcs().setBegPeriod("1");
					bpcsForm.getAddNew().getBpcs().setEndPeriod("12");
