 ...
 * @version 1.0
 */
public class BpcsUpdate extends TCGMAction
{
	/**
	 * Default Constructor
	 */
	...
	public ActionForward perform(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response)throws IOException, ServletException
	{
		...
		else if(this.isSessionValid(request))
		{
			if(this.isModelSelected(request))
			{
				...
				try
				{
					if(bpcsForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_UPDATE_ALL) )
					{
						bpcsMngr.addAllBpcsToTrans(this.getUserToken(request),bpcsForm.getSearchObject(),TCGMConstants.ACT_CD_CHG);
					}
					else if(bpcsForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_UPDATE_SELECTED) )
					{
						bpcsMngr.addSelectedBpcsToTrans(this.getUserToken(request),bpcsForm.getBpcsList(),TCGMConstants.ACT_CD_CHG);
					}
					...
					bpcsForm.getSearchObject().setModelIdInt(this.getState(request).getCurrentModelId());
					bpcsForm.getSearchObject().setDatasetTableId(DBConst.DEF_DATASET_TABLE_ID);
		...
		this.logger.debug("BpcsUpdate Forward: " + this.getForward());
