 ...
 * @version 1.0
 */
public class BpcsTranMaint extends TCGMAction
{
	/**
	 * Default Constructor
	 */
	public BpcsTranMaint()
	{
		super();
	}
	...
	public ActionForward perform(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response)throws IOException, ServletException
	{
		...
		else if( this.isSessionValid(request))
		{
			...
			if(this.isModelSelected(request))
			{
				...
				BpcsTranForm bpcsTranForm = (BpcsTranForm)form;

				bpcsTranForm.processCmd(mapping,request);
				if (bpcsTranForm!=null && bpcsTranForm.getCmd().equalsIgnoreCase(TCGMConstants.URL_PARM_VAL_ADV_FILTER) )  {
						request.getSession().setAttribute("bpcsTranAdvFilter", bpcsTranForm);
						bpcsTranForm.reset(mapping,request);
				...
				try
				{
					...
					*   the oprion to view the maintenance records of any user. Start
					**************************************************************/
					...
					/*************************************************************
					*	Added by Uday on 02/04/2006 to provide the user(Analyst)
					*   the oprion to view the maintenance records of any user. End
					**************************************************************/
					...
					bpcsTranForm.getAddNew().getBpcs().setBegPeriod("1");
					bpcsTranForm.getAddNew().getBpcs().setEndPeriod("12");
					...
					bpcsTranForm.setModels(modelMngr.getModels(userToken, fm));
