 ...
 * @version 1.0
 */
public class BpcsTranSave extends TCGMAction
{
	/**
	 * Default Constructor
	 */
	...
	public ActionForward perform(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response)throws IOException, ServletException
	{
			...
			if(this.isModelSelected(request))
			{
				...
				try
				{
					...
					**************************************************************/
					...
					if(bpcsTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_ADD) )
					{
						bpcsMngr.addNewBpcsTran(userToken,bpcsTranForm.getAddNew());
						errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("success.bpcsTran.saved"));
					}
					else if(bpcsTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_SAVE_SELECTED) )
					{
						bpcsMngr.saveSelectedBpcsTran(userToken,bpcsTranForm.getBpcsTranList());
						errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("success.bpcsTran.saved"));
					}
					else if(bpcsTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_MASS_UPDATE))
					{
						bpcsMngr.massUpdate(userToken,bpcsTranForm.getSearchObject(),bpcsTranForm.getAddNew());
						if(bpcsTranForm.getBpcsTranListSize() > 0)
						{
							errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("success.bpcsTran.saved"));
						}
					}
		...
		this.logger.debug("BpcsTranSave Forward" + this.getForward());
