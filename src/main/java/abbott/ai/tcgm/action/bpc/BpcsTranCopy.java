 ...
 * @version 1.0
 */
public class BpcsTranCopy extends TCGMAction
{
	/**
	 * Default Constructor
	 */
			...
			if(this.isModelSelected(request))
			{
				...
				try
				{
					...
					**************************************************************/
					...
					if(bpcsTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_COPY_ALL) )
					{
						bpcsMngr.copyAllBpcsTran(userToken,bpcsTranForm.getSearchObject(),bpcsTranForm.getModelSelected());
					}
					else if(bpcsTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_COPY_SELECTED) )
					{
						bpcsMngr.copySelectedBpcsTran(userToken,bpcsTranForm.getBpcsTranList(),bpcsTranForm.getModelSelected());
					}
					else if(! bpcsTranForm.getCmd().trim().equals(""))
					{
						throw new TCGMException(className,methodName,"Invalid command in action");
					}
					errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("success.bpcsTran.copied"));
