 ...
 * @version 1.0
 */
public class BpcRevTranPublish extends TCGMAction
{
	/**
	 * Default Constructor
	 */
	public BpcRevTranPublish()
	{
		super();
	}
	/**
			...
			if(this.isModelSelected(request))
			{
				...
				try
				{
					...
					**************************************************************/
					...
					if(bpcRevTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_PUBLISH_SELECTED) )
					{
						bpcRevMngr.publishSelectedBpcRevTran(userToken,bpcRevTranForm.getBpcRevTranList(), true);
					}
					else if(bpcRevTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_UNPUBLISH_SELECTED) )
					{
						bpcRevMngr.publishSelectedBpcRevTran(userToken,bpcRevTranForm.getBpcRevTranList(), false);
					}
					else if(bpcRevTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_PUBLISH_ALL) )
					{
						bpcRevMngr.publishAllBpcRevTran(userToken,bpcRevTranForm.getSearchObject(), true);
					}
					else if(bpcRevTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_UNPUBLISH_ALL) )
					{
						bpcRevMngr.publishAllBpcRevTran(userToken,bpcRevTranForm.getSearchObject(), false);
					}					
					else if(! bpcRevTranForm.getCmd().trim().equals(""))
					{
						throw new TCGMException(className,methodName,"Invalid command in action");
					}
					if(bpcRevTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_PUBLISH_ALL)||
					   bpcRevTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_PUBLISH_SELECTED)){
						errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("success.bpcrevtran.published"));
					 }else{
						errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("success.bpcrevtran.unpublished"));
					 }
