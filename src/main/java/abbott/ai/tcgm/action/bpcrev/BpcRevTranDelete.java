 ...
 * @version 1.0
 */
public class BpcRevTranDelete extends TCGMAction
{
	/**
	 * Default Constructor
	 */
	public BpcRevTranDelete()
	{
		super();
	}
	/**
	...
	public ActionForward perform(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response)throws IOException, ServletException
	{
			...
			if(this.isModelSelected(request))
			{
				...
				try
				{
					...
					**************************************************************/
					...
					if(bpcRevTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_DELETE_ALL) )
					{
						
						bpcRevMngr.deleteAllBpcRevTran(userToken,bpcRevTranForm.getSearchObject());
					}
					else if(bpcRevTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_DELETE_SELECTED) )
					{
						bpcRevMngr.deleteSelectedBpcRevTran(userToken,bpcRevTranForm.getBpcRevTranList());
					}
					else if(! bpcRevTranForm.getCmd().trim().equals(""))
					{
						throw new TCGMException(className,methodName,"Invalid command in action");
					}
					errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("success.bpcrevtran.delete"));
		...
		this.logger.debug("BpcRevTranDelete Forward" + this.getForward());
