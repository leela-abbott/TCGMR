 ...
 * @version 1.0
 */
public class BpcRevTranSave extends TCGMAction
{
	/**
	 * Default Constructor
	 */
	public BpcRevTranSave()
	{
		super();
	}
	/**
	...
	public ActionForward perform(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response)throws IOException, ServletException
	{
		...
		else if(this.isSessionValid(request))
		{
			...
			if(this.isModelSelected(request))
			{
				...
				try
				{
					...
					*   the option to Save the records of any user. Start
					**************************************************************/
					...
					/*************************************************************
					*	Added by Uday on 03/21/2006 to provide the user(Analyst)
					*   the option to Save the records of any user. End
					**************************************************************/

					if(bpcRevTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_ADD) )
					{
						bpcRevMngr.addNewBpcRevTran(userToken,bpcRevTranForm.getAddNew());
						errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("success.bpcrevtran.saved"));
					}
					else if(bpcRevTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_SAVE_SELECTED) )
					{
						bpcRevMngr.saveSelectedBpcRevTran(userToken,bpcRevTranForm.getBpcRevTranList());
						errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("success.bpcrevtran.saved"));
					}
					else if(bpcRevTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_MASS_UPDATE))
					{
						bpcRevMngr.massUpdate(userToken,bpcRevTranForm.getSearchObject(),bpcRevTranForm.getAddNew());
						if(bpcRevTranForm.getBpcRevTranListSize() > 0)
						{
							errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("success.bpcrevtran.saved"));
						}
					}
		...
		this.logger.debug("BpcRevTranSave Forward" + this.getForward());
