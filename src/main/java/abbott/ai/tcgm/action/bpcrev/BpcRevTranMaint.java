 ...
 * @version 1.0
 */
public class BpcRevTranMaint extends TCGMAction
{
	/**
	 * Default Constructor
	 */
	public BpcRevTranMaint()
	{
		super();
	}
	...
	public ActionForward perform(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response)throws IOException, ServletException
	{
		...
		else if( this.isSessionValid(request))
		{
			...
			if(this.isModelSelected(request))
			{
				...
				if (bpcRevTranForm!=null && bpcRevTranForm.getCmd().equalsIgnoreCase(TCGMConstants.URL_PARM_VAL_ADV_FILTER) )  {
						request.getSession().setAttribute("bpcRevTranAdvFilter", bpcRevTranForm);
					bpcRevTranForm.reset(mapping,request);
				...
				try
				{
					...
					**************************************************************/
					...
					bpcRevTranForm.getAddNew().getBpcRev().setBegPeriod("1");
					bpcRevTranForm.getAddNew().getBpcRev().setEndPeriod("12");
					...
					bpcRevTranForm.setModels(modelMngr.getModels(userToken, fm));
