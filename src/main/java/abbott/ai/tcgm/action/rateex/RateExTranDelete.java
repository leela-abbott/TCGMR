 ...
 * @version 1.0
 */
public class RateExTranDelete extends TCGMAction
{
	/**
	 * Default Constructor
	 */
	public RateExTranDelete()
	{
		super();
	}
	/**
...
//					Username must be set first before using helper methods
					...
					**************************************************************/
 					...
 					if(rateExTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_DELETE_ALL) )
					{

						rateExMngr.deleteAllRateExTran(userToken,rateExTranForm.getSearchObject());
					}
					else if(rateExTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_DELETE_SELECTED) )
					{
						rateExMngr.deleteSelectedRateExTran(userToken,rateExTranForm.getRateExTranList());
					}
					else if(! rateExTranForm.getCmd().trim().equals(""))
					{
						throw new TCGMException(className,methodName,"Invalid command in action");
					}
					errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("success.rateextran.delete"));
		...
		this.logger.debug("RateExTranDelete Forward" + this.getForward());
