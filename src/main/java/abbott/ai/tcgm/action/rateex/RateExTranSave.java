 ...
 * @version 1.0
 */
public class RateExTranSave extends TCGMAction
{
	/**
	 * Default Constructor
	 */
	public RateExTranSave()
	{
		super();
	}
	/**
	...
	public ActionForward perform(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response)throws IOException, ServletException
	{
		...
		else if(this.isSessionValid(request))
		{
			...
			if(this.isModelSelected(request))
			{
				...
				try
				{
					...
					*   the option to Save the maintenance records of any user. Start
					**************************************************************/
					...
					/*************************************************************
					*	Added by Uday on 03/21/2006 to provide the user(Analyst)
					*   the option to Save the maintenance records of any user. End
					**************************************************************/

					if(rateExTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_ADD) )
					{
						rateExMngr.addNewRateExTran(userToken,rateExTranForm.getAddNew());
						errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("success.rateextran.saved"));
					}
					else if(rateExTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_SAVE_SELECTED) )
					{
						rateExMngr.saveSelectedRateExTran(userToken,rateExTranForm.getRateExTranList());
						errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("success.rateextran.saved"));
					}
					else if(rateExTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_MASS_UPDATE))
					{
						rateExMngr.massUpdate(userToken,rateExTranForm.getSearchObject(),rateExTranForm.getAddNew());
						if(rateExTranForm.getRateExTranListSize() > 0)
						{
							errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("success.rateextran.saved"));
						}
					}
		...
		this.logger.debug("RateExTranSave Forward" + this.getForward());
