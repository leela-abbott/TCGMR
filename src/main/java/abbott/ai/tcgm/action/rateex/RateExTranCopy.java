 ...
 * @version 1.0
 */
public class RateExTranCopy extends TCGMAction
{
	/**
	 * Default Constructor
	 */
	public RateExTranCopy()
	{
		super();
	}
	/**
		...
		else if(this.isSessionValid(request))
		{
			...
			if(this.isModelSelected(request))
			{
				...
				try
				{
					...
					*   the option to Copy(One Model to Other) the records of any user. Start
					**************************************************************/
					...
					/*************************************************************
					*	Added by Uday on 03/21/2006 to provide the user(Analyst)
					*   the option to Copy(One Model to Other) the records of any user. End
					**************************************************************/

					if(rateExTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_COPY_ALL) )
					{
						rateExMngr.copyAllRateExTran(userToken,rateExTranForm.getSearchObject(),rateExTranForm.getModelSelected());
					}
					else if(rateExTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_COPY_SELECTED) )
					{
						rateExMngr.copySelectedRateExTran(userToken,rateExTranForm.getRateExTranList(),rateExTranForm.getModelSelected());
					}
					else if(! rateExTranForm.getCmd().trim().equals(""))
					{
						throw new TCGMException(className,methodName,"Invalid command in action");
					}
					errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("success.rateextran.copied"));
