package abbott.ai.tcgm.action.salestype;
 ...
 * @version 1.0
 */
public class DeleteSalesType extends TCGMAction
{
	/**
	 * Default Constructor
	 */
	public DeleteSalesType()
	{
		super();
	}
	...
	public ActionForward perform(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response)throws IOException, ServletException
	{
		...
		else if(this.isSessionValid(request))
		{
			...
			SalesTypeForm salesTypeForm = (SalesTypeForm)form;//cast the form that was passed in to the correct type for this action

			salesTypeForm.processCmd(mapping,request);

			SalesTypeMngr salesTypeMngr = new SalesTypeMngr(); //create the helper class that will handle the processing
			//CurrencyCodeMngr currCodeMngr = new CurrencyCodeMngr();

			try
			{
				salesTypeMngr.deleteSalesType(userToken,salesTypeForm.getSlsTypeList());
				salesTypeForm.setSlsTypeToEdit(new SalesType());
				salesTypeForm.setSearchObject(new SalesType());
				salesTypeForm.setSlsTypeList(salesTypeMngr.getSalesType(userToken,salesTypeForm.getSortObject()));
