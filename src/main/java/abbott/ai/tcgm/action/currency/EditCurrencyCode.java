 ...
 * @version 1.0
 */
public class EditCurrencyCode extends TCGMAction
{
	/**
	 * Default Constructor
	 */
	public EditCurrencyCode()
	{
		super();
	}
	...
	public ActionForward perform(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response)throws IOException, ServletException
	{
		...
		else if(this.isSessionValid(request))
		{
			...
			try
			{
				...
				currencyCodeForm.setCurrencyCodeToEdit(currencyCodeMngr.getCurrencyByCode(userToken,currencyCodeForm.getSearchObject()));
				this.setForward(TCGMConstants.FORWARD_SUCCESS);
			}
			catch(TCGMException tcgme)
			{
				this.logger.error(tcgme.toString(),tcgme);
				//this.errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("exception.currency.edit"));
