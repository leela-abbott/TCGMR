 ...
 * @version 1.0
 */
public class SaveCurrencyCode extends TCGMAction
{
	/**
	 * Default Constructor
	 */
	public SaveCurrencyCode()
	{
		super();
	}
	...
	public ActionForward perform(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response)throws IOException, ServletException
	{
		...
		else if(this.isSessionValid(request))
		{
			...
			try
			{
				currencyCodeMngr.saveCurrencyCode(userToken,currencyCodeForm.getCurrencyCodeToEdit());
				currencyCodeForm.setCurrencyCodeToEdit(new CurrencyCode());
				currencyCodeForm.setSearchObject(new CurrencyCode());
				currencyCodeForm.setCurrencylist(currencyCodeMngr.getCurrencyCode(userToken,currencyCodeForm.getSortObject()));
				this.setForward(TCGMConstants.FORWARD_SUCCESS);
			}
			catch(TCGMException tcgme)
			{
				this.logger.error(tcgme.toString(),tcgme);
				//this.errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("exception.currency.save"));
