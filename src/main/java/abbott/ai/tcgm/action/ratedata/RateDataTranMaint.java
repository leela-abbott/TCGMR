 ...
 * @version 1.0
 */
public class RateDataTranMaint extends TCGMAction
{
	/**
	 * Default Constructor
	 */
	public RateDataTranMaint()
	{
		super();
	}
			...
			if(this.isRateSetSelected(request))
			{
				...
				try
				{
					...
					**************************************************************/
					...
					rateDataTranForm.initModelAndDataset(this.getState(request).getCurrentModelIdString(),Integer.toString(state.getCurRateSetTableId()) );
					...
					rateDataTranForm.getAddNew().getRateData().setBegPeriod("1");
					rateDataTranForm.getAddNew().getRateData().setEndPeriod("12");
