 ...
 * @version 1.0
 */
public class RateDataTranPublish extends TCGMAction
{
	/**
	 * Default Constructor
	 */
	public RateDataTranPublish()
	{
		super();
	}
	/**
			...
			if(this.isRateSetSelected(request))
			{
				...
				try
				{
					...
					**************************************************************/
					...
					if(rateDataTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_PUBLISH_SELECTED) )
					{
						rateDataMngr.publishSelectedRateDataTran(userToken,rateDataTranForm.getRateDataTranList());
					}
					else if(rateDataTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_PUBLISH_ALL) )
					{
						rateDataMngr.publishAllRateDataTran(userToken,rateDataTranForm.getSearchObject());
					}
					else if(! rateDataTranForm.getCmd().trim().equals(""))
					{
						throw new TCGMException(className,methodName,"Invalid command in action");
					}
					errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("success.ratedatatran.published"));
