 ...
 * @version 1.0
 */
public class RateDataTranCopy extends TCGMAction
{
	/**
	 * Default Constructor
	 */
	public RateDataTranCopy()
	{
		super();
	}
	/**
			...
			if(this.isRateSetSelected(request))
			{
				...
				try
				{
					...
					**************************************************************/
					...
					if(rateDataTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_COPY_ALL) )
					{
// 4/21/03 Copy function Not Applicable in Rate Data
//						rateDataMngr.copyAllRateDataTran(userToken,rateDataTranForm.getSearchObject(),rateDataTranForm.getModelSelected());
					}
					else if(rateDataTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_COPY_SELECTED) )
					{
// 4/21/03 Copy function Not Applicable in Rate Data
//						rateDataMngr.copySelectedRateDataTran(userToken,rateDataTranForm.getRateDataTranList(),rateDataTranForm.getModelSelected());
					}
					else if(! rateDataTranForm.getCmd().trim().equals(""))
					{
						throw new TCGMException(className,methodName,"Invalid command in action");
					}
					errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("success.ratedatatran.copied"));

					//rateDataTranForm.initModelAndDataset(this.getState(request).getCurrentModelIdString(),DBConst.DEF_DATASET_TABLE_ID);
					//rateDataTranForm.getSearchObject().setModelIdInt(state.getRateModelId());
					//rateDataTranForm.getSearchObject().setDatasetTableIdInt(state.getCurRateSetTableId());
					rateDataTranForm.getSearchObject().getRateData().setDatasetTableIdInt(state.getCurRateSetTableId());
					//rateDataTranForm.getAddNew().getRateData().setModelIdInt(this.getState(request).getCurrentModelId());
					//rateDataTranForm.getAddNew().getRateData().setDatasetTableId(DBConst.DEF_DATASET_TABLE_ID);
					...
					//rateDataTranForm.setModels(modelMngr.getModels(userToken, new FactorModel()));
					//rateDataTranForm.setModelSelected(TCGMConstants.NONE);
