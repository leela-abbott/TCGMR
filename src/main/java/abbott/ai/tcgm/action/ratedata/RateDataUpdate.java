 ...
 * @version 1.0
 */
public class RateDataUpdate extends TCGMAction
{
	/**
	 * Default Constructor
	 */
	public RateDataUpdate()
	{
		super();
	}
	/**
	...
	public ActionForward perform(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response)throws IOException, ServletException
	{
		...
		else if(this.isSessionValid(request))
		{
			if(this.isRateSetSelected(request))
			{
				...
				try
				{
					if(rateDataForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_UPDATE_ALL) )
					{
						rateDataMngr.addAllRateDataToTrans(userToken,rateDataForm.getSearchObject(),TCGMConstants.ACT_CD_CHG);
					}
					else if(rateDataForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_UPDATE_SELECTED) )
					{
					...
					errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("success.ratedata.insert"));
