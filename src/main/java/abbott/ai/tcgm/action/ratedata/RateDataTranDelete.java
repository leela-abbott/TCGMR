 ...
 * @version 1.0
 */
public class RateDataTranDelete extends TCGMAction
{
	/**
	 * Default Constructor
	 */
	public RateDataTranDelete()
	{
		super();
	}
	/**
	...
	public ActionForward perform(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response)throws IOException, ServletException
	{
			...
			if(this.isRateSetSelected(request))
			{
				...
				try
				{
					...
					**************************************************************/
					...
					if(rateDataTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_DELETE_ALL) )
					{
						rateDataMngr.deleteAllRateDataTran(userToken,rateDataTranForm.getSearchObject());
					}
					else if(rateDataTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_DELETE_SELECTED) )
					{
						rateDataMngr.deleteSelectedRateDataTran(userToken,rateDataTranForm.getRateDataTranList());
					}
					else if(! rateDataTranForm.getCmd().trim().equals(""))
					{
						throw new TCGMException(className,methodName,"Invalid command in action");
					}
					errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("success.ratedatatran.delete"));
		...
		this.logger.debug("RateDataTranDelete Forward" + this.getForward());
