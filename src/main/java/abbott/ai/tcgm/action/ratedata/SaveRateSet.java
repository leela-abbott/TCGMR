 ...
 * @version 1.0
 */
public class SaveRateSet extends TCGMAction
{
	/**
	 * Default Constructor
	 */
	...
	public ActionForward perform(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response)throws IOException, ServletException
	{
		...
		else if(this.isSessionValid(request))
		{
			...
			try
			{
				if(rateSetForm.getCopyFromDatasetTableId().equals(TCGMConstants.NONE_SELECTED))
				{
					datasetMngr.saveDataset(this.getUserToken(request),rateSetForm.getEditRateSet(),DBConst.RATE_MODEL_NAME);
				}
				else //copying
				{
					datasetMngr.copyRateSet(this.getUserToken(request),rateSetForm,DBConst.RATE_MODEL_NAME);
				}
			...
			catch(TCGMException tcgme)
			{
				...
				//this.errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("exception.mngratesets.save"));
