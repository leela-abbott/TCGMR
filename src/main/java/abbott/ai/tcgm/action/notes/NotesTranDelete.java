 ...
 * @version 1.0
 */
public class NotesTranDelete extends TCGMAction {
	/**
	 * Default Constructor
	 */
	public NotesTranDelete() {
		super();
	}
	/**
	 *
	...
	public ActionForward perform(
		...
		if (form == null) {
			...
			errors.add(
				...
				new ActionError("error.notestran.form.missing"));
			...
			if (this.isModelSelected(request)) {
				...
				try {
					notesTranForm.initModelAndDataset(
				...
				**************************************************************/
					...
					if (notesTranForm
						...
						notesMngr.deleteAllNotesTran(
							userToken,
							notesTranForm.getSearchObject());
					} else if (
						notesTranForm.getCmd().equals(
							TCGMConstants.URL_PARM_VAL_DELETE_SELECTED)) {
						notesMngr.deleteSelectedNotesTran(
							userToken,
							notesTranForm.getNotesTranList());
					...
					errors.add(
						ActionErrors.GLOBAL_ERROR,
						new ActionError("success.notestran.delete"));
					...
					try {
						notesTranForm.initModelAndDataset(
							this.getState(request).getCurrentModelIdString(),
							DBConst.DEF_DATASET_TABLE_ID);
						notesTranForm.getPagingFilter().setTotalRecordsInSet(
							notesMngr.getCount(
								userToken,
								notesTranForm.getSearchObject()));
						notesTranForm.setNotesTranList(
							notesMngr.getNotesTran(
								userToken,
								notesTranForm.getSearchObject(),
								notesTranForm.getPagingFilter(),
								notesTranForm.getSortObject()));
						notesTranForm.getSearchObject().getNotes().getCreateLog().setUserName(userToken.getUserid()); //03/23/2006 Udaya B Aravapalli								
						this.setForward(TCGMConstants.FORWARD_SUCCESS);
