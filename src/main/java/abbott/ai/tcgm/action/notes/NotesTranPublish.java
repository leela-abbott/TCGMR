 ...
 * @version 1.0
 */
public class NotesTranPublish extends TCGMAction
{
	/**
	 * Default Constructor
	 */
	public NotesTranPublish()
	{
		super();
	}
	/**
			...
			if(this.isModelSelected(request))
			{
				...
				try
				{
					...
					**************************************************************/
					...
					if(notesTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_PUBLISH_SELECTED) )
					{
						notesMngr.publishSelectedNotesTran(userToken,notesTranForm.getNotesTranList(), true);
					}
					else if(notesTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_UNPUBLISH_SELECTED) )
					{
						notesMngr.publishSelectedNotesTran(userToken,notesTranForm.getNotesTranList(), false);
					}
					else if(notesTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_PUBLISH_ALL) )
					{
						notesMngr.publishAllNotesTran(userToken,notesTranForm.getSearchObject(), true);
					}
					else if(notesTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_UNPUBLISH_ALL) )
					{
						notesMngr.publishAllNotesTran(userToken,notesTranForm.getSearchObject(), false);
					}
					else if(! notesTranForm.getCmd().trim().equals(""))
					{
						throw new TCGMException(className,methodName,"Invalid command in action");
					}
					if(notesTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_PUBLISH_ALL)||
					   notesTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_PUBLISH_SELECTED)){
							errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("success.notestran.published"));
					 }else{
						    errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("success.notestran.unpublished"));
					 }
