 ...
 * @version 1.0
 */
public class NotesTranCopy extends TCGMAction
{
	/**
	 * Default Constructor
	 */
	public NotesTranCopy()
	{
		super();
	}
	/**
			...
			if(this.isModelSelected(request))
			{
				...
				try
				{
					...
					**************************************************************/
					...
					if(notesTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_COPY_ALL) )
					{
						notesMngr.copyAllNotesTran(userToken,notesTranForm.getSearchObject(),notesTranForm.getModelSelected());
					}
					else if(notesTranForm.getCmd().equals(TCGMConstants.URL_PARM_VAL_COPY_SELECTED) )
					{
						notesMngr.copySelectedNotesTran(userToken,notesTranForm.getNotesTranList(),notesTranForm.getModelSelected());
					}
					else if(! notesTranForm.getCmd().trim().equals(""))
					{
						throw new TCGMException(className,methodName,"Invalid command in action");
					}
					errors.add(ActionErrors.GLOBAL_ERROR,new ActionError("success.notestran.copied"));
