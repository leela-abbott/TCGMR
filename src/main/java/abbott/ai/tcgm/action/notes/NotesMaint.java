 ...
 * @version 1.0
 */
public class NotesMaint extends TCGMAction
{
	/**
	 * Default Constructor
	 */
	...
	public ActionForward perform(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response)throws IOException, ServletException
	{
		...
		else if(this.isSessionValid(request))
		{
			if(this.isModelSelected(request))
			{
				...
				if (notesForm!=null && notesForm.getCmd().equalsIgnoreCase(TCGMConstants.URL_PARM_VAL_ADV_FILTER) )  {
						request.getSession().setAttribute("notesTranAdvFilter", notesForm);
						notesForm.reset(mapping,request);
				...
				try
				{
					...
					if(notesForm.getErrs().equalsIgnoreCase("on")){
					...
					if (notesForm.getCmd().equalsIgnoreCase(TCGMConstants.URL_PARM_VAL_EMPTY))
					{
					...
					notesForm.setNotesErrorList(new Vector());
					}
