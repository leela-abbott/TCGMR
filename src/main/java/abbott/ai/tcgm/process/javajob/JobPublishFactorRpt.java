...
public class JobPublishFactorRpt implements JavaJob {

	public JobPublishFactorRpt()  {
	}
     ...
     * Modified On : 01/05/2006
     * Description : This code will do the following functionality
     *               a.Insert / Update records(Model_Id and Model_Version) in the Published_Factor_Rpts table.   
     *               b.Execute the Stored Proc SEND_FACRPT   
     *               c.Execute the Stored Proc CPY2_PUB.
     ********************************************************************************/
	    String modelId = jobInstance.getModelId();
		UserToken ut = AppConst.getInstance().getJobId();

        ProcessMngr pm = new ProcessMngr();		
		pm.executeJobStep("SEND_FACRPT", modelId, "-1", ut, parms); // Execute Stored Proc: SEND_FACRPT
		pm.executeJobStep("CPY2_PUB", modelId, "-1", ut, parms);    // Execute Stored Proc: CPY2_PUB
	}
}